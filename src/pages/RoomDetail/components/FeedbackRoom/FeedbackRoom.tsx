...
export default function FeedbackRoom(props:any) {
  ...
  const onSubmitComment = async (values:any, { resetForm }  :any) => {
  ...
  const renderFeedBack = () => {
    if (showAllComments) {
      return feedBackRoom.map((element:any) => (
      ...
      return feedBackRoom.slice(0, 4).map((element:any) => (
        <div key={element.id} className="mb-5 p-4 bg-white rounded-lg shadow-md">
          <div className="flex items-center">
            <div>
              <img
            ...
            <div className="ml-4">
              <h4 className="font-bold text-lg text-gray-900">{element.tenNguoiBinhLuan}</h4>
              <span className="text-sm text-gray-500">{formatDate(element.ngayBinhLuan)}</span>
            </div>
          </div>
          <div className="text-gray-800 tracking-wider mt-4">
