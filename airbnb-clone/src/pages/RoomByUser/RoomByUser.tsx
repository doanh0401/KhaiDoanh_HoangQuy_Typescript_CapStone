...
export default function RoomByUser() {
  ...
  const [roomTicket, setRoomTicket] = useState<any>([]);
  const [roomTicketDetail, setRoomTicketDetail] = useState<any>([]);
  ...
  const totalCost = roomTicket.reduce((acc:any, ticket:any, index:number) => {
        ...
        <div>
          ...
          <div className='flex'>
            <div className='flex flex-col mt-4 md:pr-5 lg:w-2/3'>
              {roomTicket.map((ticket:any, index:number) => (
